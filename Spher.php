...
if ($_SERVER['REQUEST_METHOD'] === 'POST')
{
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        $stmt_check = $conn->prepare("SELECT * FROM Список_Сфера WHERE (start_date >= ? AND start_date <= ?) OR (end_date >= ? AND end_date <= ?)");
        ...
        {
            $stmt_marb = $conn->prepare("INSERT INTO Список_Сфера (ФИО, Email, start_date, end_date, Дом) VALUES (?, ?, ?, ?, 'Сфера')");
            $stmt_marb->bind_param("ssss", $full_name, $email, $start_date, $end_date);

            $stmt_oher = $conn->prepare("INSERT INTO Список_бронирований (ФИО, Email, start_date, end_date, Дом) VALUES (?, ?, ?, ?, 'Сфера')");
            ...
            if($stmt_marb->execute() && $stmt->execute() && $stmt_oher->execute())
            {
                ...
                $message = 'Новое бронирование в дом Сфера от ' . $full_name . ' На период c ' . $start_date . ' по ' . $end_date;
